...
class VDSR(object):
    def __init__(self, X, Y, p):
        ...
        with tf.variable_scope('layer1'):
            weights = tf.get_variable("w",[3,3,3,64], initializer=WEIGHT_INITIALIZER1)
        ...
        for i in range(p.depth-2):
            ...
            with tf.variable_scope(scope):
                weights = tf.get_variable("w",[3,3,64,64],initializer=WEIGHT_INITIALIZER2)
                bias = tf.get_variable("b",[64],initializer=tf.constant_initializer(0))
                self.weights.append(weights)
                self.weights.append(bias)
        ...
        with tf.variable_scope("layer"+str(p.depth)):
            weights = tf.get_variable("w",[3,3,64,3],initializer=WEIGHT_INITIALIZER2)
