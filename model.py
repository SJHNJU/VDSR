...
important attributes:
    ...
    weight initializer
'''
#WEIGHT_INITIALIZER1 = tf.random_normal_initializer(stddev=np.sqrt(2.0/9))
#WEIGHT_INITIALIZER2 = tf.random_normal_initializer(stddev=np.sqrt(2.0/9/64))
...
class VDSR(object):
    def __init__(self, X, Y, p):
        ...
        with tf.variable_scope('layer1'):
            weights = tf.get_variable("w",[3,3,1,64], initializer=WEIGHT_INITIALIZER)
        ...
        with tf.variable_scope("layer"+str(p.depth)):
            weights = tf.get_variable("w",[3,3,64,1],initializer=WEIGHT_INITIALIZER)
            bias = tf.get_variable("b",[1],initializer=tf.constant_initializer(0))
            self.weights.append(weights)
            self.weights.append(bias)
            self.output = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(self.output, weights, strides=[1,1,1,1], padding='SAME'), bias))
        ...
        #MSE
        self.loss = tf.reduce_sum(tf.abs(Y-self.output))
